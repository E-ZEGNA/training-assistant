import { hasVerbatimLeak, looksLikeExfiltration, retrieveChunks } from './retrieval.js';
...
function rawBoundaryBeforeHeldText(text, heldCharacters = 99) {
  const normalizedEnds = [];
  const words = [...text.matchAll(/\S+/gu)];
  for (let wordIndex = 0; wordIndex < words.length; wordIndex += 1) {
    const match = words[wordIndex];
    if (wordIndex > 0) normalizedEnds.push(match.index);
    for (let offset = 0; offset < match[0].length; offset += 1) {
      normalizedEnds.push(match.index + offset + 1);
    }
  }
  const safeCharacters = normalizedEnds.length - heldCharacters;
  return safeCharacters > 0 ? normalizedEnds[safeCharacters - 1] : 0;
}
...
export async function generateInterviewAnswer({ config, master, session, transcriptContext, signal, onToken, onReplace }) {
  ...
  const recentHistory = session.answerHistory.slice(-3).map((item) => `问题：${item.question}\n回答：${item.answer}`).join('\n\n');
  const system = [
  ...
  let answer = '';
  let releasedRawCharacters = 0;
  let contentStarted = false;
  let blocked = false;
  for await (const token of stream) {
    answer += token;
    if (!contentStarted) {
      const firstContent = answer.search(/\S/u);
      if (firstContent < 0) continue;
      releasedRawCharacters = firstContent;
      contentStarted = true;
    }
    if (hasVerbatimLeak(answer, master.text)) {
      blocked = true;
      break;
    }
    const safeBoundary = rawBoundaryBeforeHeldText(answer);
    if (onToken && safeBoundary > releasedRawCharacters) {
      await onToken(answer.slice(releasedRawCharacters, safeBoundary));
      releasedRawCharacters = safeBoundary;
    }
  }
  ...
  } else if (onToken && rawAnswer.length > releasedRawCharacters) {
    await onToken(rawAnswer.slice(releasedRawCharacters).trimEnd());
  }
  if (!answer) throw new Error('LLM returned an empty answer');
  session.answerHistory.push({ question: currentQuestion.slice(-2000), answer, at: Date.now() });
  if (session.answerHistory.length > 8) session.answerHistory.shift();
  return answer;
}
